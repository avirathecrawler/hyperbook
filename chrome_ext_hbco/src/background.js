...
settings = {
    ...
    set the_notes(val) {
      ...
      get the_channel() {
      return localStorage['the_channel'];
    },
    set the_channel(val) {
      localStorage['the_channel'] = val;
    }
...
function trigger_channel(url){
  if( url in TRIGINFO   )
    {
      console.log("TRIGGERED="+url);
      settings.the_channel = TRIGINFO[url];
    }
}
...
function TriggerDB()
{
      ajax_call({action:"TRIGGERDB"},function(res){
        console.log(res);
        TRIGINFO= $.parseJSON(res);
        //console.log(TRIGINFO["http://yourstory.com/"]);
